    ...
    useEffect(() => {
        ...
        const start = async () => {
            try {
                ...
                // 3. Init MediaPipe Hands (reuse singleton)
                ...
                if (!_mpHandsInstance) {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const instance = new MP({ locateFile: (f: string) => `https://cdn.jsdelivr.net/npm/@mediapipe/hands@0.4.1646424915/${f}` }) as any;
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (handsRef.current as any).onResults((results: any) => {
                    if (cancelled) return;
                ...
                const loop = async () => {
                    ...
                    if (videoRef.current.readyState >= 2) {
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        try { await (handsRef.current as any).send({ image: videoRef.current }); } catch { }
                    }
