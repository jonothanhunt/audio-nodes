...
// Param configuration
interface SelectParamConfig { key: 'preset' | 'waveform'; type: 'select'; options: readonly string[]; default: string; variant: 'string'; }
interface NumberParamConfig { key: 'attack' | 'decay' | 'sustain' | 'release' | 'cutoff' | 'resonance' | 'glide' | 'gain' | 'maxVoices'; type: 'number'; min: number; step: number; default: number; max?: number; }

type SynthParamConfig = SelectParamConfig | NumberParamConfig;

const synthParamConfig: readonly SynthParamConfig[] = [
  { key: 'preset', type: 'select', options: ['Init','Pluck','Pad','Bass'] as const, default: 'Init', variant: 'string' },
  { key: 'waveform', type: 'select', options: ['sine','sawtooth','square','triangle'] as const, default: 'sawtooth', variant: 'string' },
  { key: 'attack', type: 'number', min: 0, step: 0.001, default: 0.005 },
  { key: 'decay', type: 'number', min: 0, step: 0.001, default: 0.12 },
  { key: 'sustain', type: 'number', min: 0, max: 1, step: 0.01, default: 0.7 },
  { key: 'release', type: 'number', min: 0, step: 0.001, default: 0.12 },
  { key: 'cutoff', type: 'number', min: 20, max: 20000, step: 1, default: 10000 },
  { key: 'resonance', type: 'number', min: 0, max: 1, step: 0.01, default: 0.2 },
...
export default function SynthesizerNode({ id, data, selected }: SynthNodeProps) {
  ...
  React.useEffect(() => {
    synthParamConfig.forEach(p => {
      ...
      if (current == null) onParameterChange(id, p.key, p.default as number | string);
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const numericKeys = synthParamConfig.filter(p => p.type === 'number').map(p => p.key);
  const stringKeys = synthParamConfig.filter(p => p.type === 'select').map(p => p.key);

  const getValue = (key: SynthParamKey) => (data as Record<string, unknown>)[key];

  return (
    <NodeUIProvider accentColor={accentColor} numericKeys={numericKeys} stringKeys={stringKeys}>
      {selected && (
        ...
        <div className="grid grid-cols-[minmax(16rem,_auto)_auto] gap-y-2 gap-x-4">
          <div className="space-y-2 col-span-1">
            ...
            {synthParamConfig.map(cfg => {
              const prettyLabel = cfg.key === 'maxVoices' ? 'Voices' : (cfg.key.charAt(0).toUpperCase() + cfg.key.slice(1));
              if (cfg.type === 'select') {
                return (
                  <SelectParam
                    key={cfg.key}
                    ...
                    paramKey={cfg.key}
                    label={prettyLabel}
                    value={String(getValue(cfg.key) ?? cfg.default)}
                    options={[...cfg.options]}
