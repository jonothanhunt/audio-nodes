...
interface BooleanParamProps {
  ...
  label: string;
  value: boolean;
  onParameterChange: (nodeId: string, parameter: string, value: boolean) => void;
}

export function BooleanParam({ nodeId, paramKey, label, value, onParameterChange }: BooleanParamProps) {
  const stop = (e: React.SyntheticEvent) => { e.stopPropagation(); };
  return (
    <ParamRow label={label} paramKey={paramKey}>
      <button
        onClick={(e) => { e.stopPropagation(); onParameterChange(nodeId, paramKey, !value); }}
        onPointerDown={stop}
        onMouseDown={stop}
